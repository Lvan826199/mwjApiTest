...
from mwjApiTest.core.env_prepare import ENV, make_data_tools, DB
from mwjApiTest.core.basecase import BaseTestCase
from mwjApiTest.core.basecase import CaseLog
...
class CaseData:
    ...
    # requests 源码地址 https://docs.python-requests.org/en/latest/api/
    # 源码在 requests库 的 api 对应的 request方法
    # All of Requests’ functionality can be accessed by these 7 methods. They all return an instance of the Response object.
    # 所有请求的功能都可以通过这7个方法访问。它们都返回一个Response对象的实例。
...
class HttpCase(BaseTestCase, Extract, CaseLog):
    ...
    def __get_db_check(self, case):
        ...
        func = getattr(make_data_tools, hook)
        if not func:
            raise ValueError('函数引用错误：\n{}\n中的函数{}未定义！,'.format(make_data_tools, hook))
        return func
...
class Request:
    ...
    def request_api(self):
        ...
        try:
            ...
            self.test_method = self.request_data.get('method')
        ...
        self.test.status_code = response.status_code
        ...
        self.request_log(self.test)
        return response

    def request_log(self,test):
